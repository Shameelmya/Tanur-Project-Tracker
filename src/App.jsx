...
const INITIAL_SUB_FOLDERS = [
  { id: 'priority', mainFolderId: 'mf_priority', name: 'Priority Files', color: 'from-red-500 to-rose-700', theme: 'rose', iconName: 'AlertTriangle' },
  ...
  { id: 'personal', mainFolderId: 'mf_personal', name: 'Personal', color: 'from-violet-500 to-fuchsia-600', theme: 'fuchsia', iconName: 'User' },
  { id: 'common', mainFolderId: 'mf_common', name: 'Common', color: 'from-slate-500 to-slate-700', theme: 'slate', iconName: 'Users' },
  { id: 'trivandrum', mainFolderId: 'mf_trivandrum', name: 'Trivandrum Office', color: 'from-cyan-500 to-blue-700', theme: 'blue', iconName: 'Building2' },
];
...
export default function App() {
  ...
  customCategories.forEach(cat => {
    ...
    else displayBodiesMap.set(cat.id, { ...(displayBodiesMap.get(cat.id) || {}), ...cat, icon: ICON_MAP[cat.iconName] || Folder });
  });
  const allSubFolders = Array.from(displayBodiesMap.values()).map(sb => ({ ...sb, icon: sb.icon || ICON_MAP[sb.iconName] || Folder }));
    ...
    <div className="h-screen bg-slate-50 text-slate-800 flex flex-col overflow-hidden" style={{ fontFamily: "'Sora', 'Noto Serif Malayalam', sans-serif" }}>
      ...
      <header className="bg-white shadow-sm shrink-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
          ...
          <span className="text-[10px] sm:text-xs font-semibold bg-emerald-50 border border-emerald-100 text-emerald-800 px-2.5 py-1.5 rounded-full hidden sm:block">
